/*
 * Audience Chip support ported from GB I896 source
 * by Michael Carey (mtcarey:xda-developers) 12/2011
*/
...
//	msleep(50);
	...
	debug("A1026Loadfirmware called");
...
//ldj_20100719
	...
	msleep(50);
//ldj_20100719
	debug("A1026 powerup called");
	debug("A1026 HWREV: %d",HWREV);
	if((HWREV == 0x08) || (HWREV == 0x04))
	{
...
//					printk("0x%x \n",buf_bypass_tuning[i]);
		...
		case NS0:
			{
			debug("A1026SetFeature NSO mode");
...
static int A1026_probe (struct i2c_client *client)
{
    ...
    if( strcmp(client->name, "A1026_driver") != 0 )
    {
        ret = -1;
